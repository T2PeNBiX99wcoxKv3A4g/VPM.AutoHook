...
namespace io.github.ykysnk.autohook.Editor;

// Refs: https://gist.github.com/LotteMakesStuff/d6a9a4944fc667e557083108606b7d22
[CustomPropertyDrawer(typeof(AutohookAttribute))]
public class AutohookAttributeDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        // First, let's attempt to find a valid component we could hook into this property
        var component = FindAutohookTarget(property);
        if (component)
        {
            // if we found something, AND the autohook is empty, let's slot it.
            // the reason were straight up looking for a target component is so we
            // can skip drawing the field if there's a valid autohook. 
        ...
        // haven't found one? let's just draw the default property field, let the user manually
        // hook something in.
    ...
    {
        // if there's a valid autohook target, we skip drawing, so the height is zeroed
        var component = FindAutohookTarget(property);
        if (component)
            return 0;
    ...
    /// <returns></returns>
    private static Component? FindAutohookTarget(SerializedProperty property)
    {
